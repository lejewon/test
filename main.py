class Hangman():
    ...
    def is_invalid_letter(self, input_):
        ...
        """
        return input_.isdigit() or (input_.isalpha() and input_ >= 1)
    ...
    def play(self):
        ...
        """
        while True:
            self.print_gmae_status() # 현재 게임 상태를 먼저 출력해줌. 
            user_input = self.get_user_input() # user_input 을 입력받는 메소드 적용. 

            # Validate the user input
            if user_input.is_invalid_letter() : # 플레이어가 입력한 인풋이 유효한 값인지 아닌지. 
                print('제대로 입력하시오')# 아니면, 제대로 입력하라는 메세지 출력하고
                continue# break or continue 중 하나를 사용하여 계속 입력 받음. 
            # Check if the letter is not already guessed
            if user_input in update_progress() :
               print('이미 입력받았던 알파벳') # 이미 입력받았던 알파벳이면, 이미 입력했다 메시지 출력
            ...
            if user_input in word_to_guess:
                indexes = find_indexs(user_input) # 추측한 글자의 인덱스 받기 
                update_progress(user_input,indexs) # 입력받은 글자로 현재 게임 상태 업데이트 

                # If there is no letter to find in the word
                if '_' == 0: # 더이상 맞출 글자가 없으면 
                    print('\n------Yay! You win!')
